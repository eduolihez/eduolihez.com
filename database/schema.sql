...
UPDATE `posts`
SET `content` = CONCAT(`content`, '\r\n<p><strong>Relacionado:</strong> <a href="/blog/post/?slug=vigilar-el-catalogo-kev-de-cisa-sin-servidor">automaticé la vigilancia del propio catálogo KEV</a> para no depender de acordarme de consultarlo.</p>')
WHERE `slug` = 'priorizar-cves-sin-fiarse-solo-del-cvss'
  AND `content` NOT LIKE '%vigilar-el-catalogo-kev-de-cisa-sin-servidor%';

UPDATE `posts`
SET `content` = CONCAT(`content`, '\r\n<p><strong>Relacionado:</strong> el mismo principio de minimizar lo que se confía en manos ajenas aparece en <a href="/blog/post/?slug=revocar-un-token-sin-derribar-la-sesion-entera">BinCat</a>, para revocar un token sin derribar toda la sesión.</p>')
...
UPDATE `posts`
SET `content` = CONCAT(`content`, '\r\n<p><strong>Relacionado:</strong> <a href="/blog/post/?slug=el-desacuerdo-entre-clasificadores-pierde-contra-algo-mas-simple">medí después si el desacuerdo entre varios clasificadores ayuda a priorizar qué correo revisar</a> — la respuesta no fue la esperada.</p>')
